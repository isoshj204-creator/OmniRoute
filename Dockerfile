...
RUN --mount=type=cache,id=s/b084d9a5-5d37-4bcb-afcf-a8fd8867f911-/var/cache/apt,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=s/b084d9a5-5d37-4bcb-afcf-a8fd8867f911-/var/lib/apt/lists,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
  && apt-get upgrade -y \
...
# broken image.
RUN --mount=type=cache,id=s/b084d9a5-5d37-4bcb-afcf-a8fd8867f911-/root/.npm,target=/root/.npm \
...
COPY . ./
RUN --mount=type=cache,id=s/b084d9a5-5d37-4bcb-afcf-a8fd8867f911-/app/.build/next/cache,target=/app/.build/next/cache \
  mkdir -p /app/data \
  && npm run build \
